  ...
  return (
    <aside className={'fixed top-0 left-0 z-40 h-screen bg-s1 border-r border-bdr flex flex-col shrink-0 transition-all duration-300 ' + (minimized ? 'w-16' : 'w-64')}>
      ...
      <nav className="flex-1 px-2 overflow-y-auto overflow-x-hidden">
        ...
        <button onClick={onNewCourse} className={'w-full flex items-center gap-3 rounded-lg text-sm text-muted hover:text-accent transition-colors mt-1 ' + (minimized ? 'p-2.5 justify-center' : 'px-3 py-2')} title={minimized ? 'New Course' : ''}>
